...
public class MyPlan implements Cloneable{
    ...
    public MyPlan(int nbVehicles_) {
        ...
        for(int i = 0; i < nbVehicles_; i++) {
            repartition.put(i, new LinkedList<MyTask>()); // We initialize with an empty linked list
        }
    }
    ...
    public void addTask(int vehicleId, Task task) {
        ...
        // Giving only one pickup or one delivery makes no sense, we would have an invalid state
        MyTask pickup = new MyTask(task, true);
        MyTask delivery = new MyTask(task, false);

        Random r = new Random();
        if (this.repartition.get(vehicleId).size()>0 && false){
            int i = r.nextInt(this.repartition.get(vehicleId).size()-1);
            int j = i + r.nextInt(this.repartition.get(vehicleId).size()-i-1);

            this.repartition.get(vehicleId).add(i, pickup);
            this.repartition.get(vehicleId).add(j, delivery);
        } else {
    ...
    public boolean isValid(Vehicle[] vehicles, int nbTasks) {
            ...
            for (MyTask task : vehicleTasks) {
                if(task.isPickup()) {
                    ...
                    if(load > vehicles[i].capacity()) {
                        //System.err.println("Load too heavy : " + i);
                        return false;
                    }
                    if(alreadyPickedUpTasks.contains(task.getTask().id)){
                        //System.err.println("Task already picked up : " + task.getTask().id);
                        return false;
                    }
                    ...
                    if(alreadyDeliveredTasks.contains(task.getTask().id)){
                        //System.err.println("Already delivered" + task.getTask().id);
                        return false;
                    }
                    if (!carriedTasks.contains(task.getTask().id)){
                        //System.err.println("Task not carried " + task.getTask().id);
                        return false;
                    }
        ...
        if(totalTasks / 2 != nbTasks) {
            //System.err.println("Tasks missing");
            return false;
        }
